...
def handle_test_creation(request):
    if request.method == 'POST':  # If the form has been submitted...
        ...
        if form.is_valid():  # All validation rules pass
            ...
            all_words = Word.objects.filter(dictionary_id=dictionary.id, theme_id__in=themes_ids)
            print(all_words)
            if len(all_words) == 0:
                return HttpResponse("Here's no words in this dictionary for this themes yet.Please choose another one.")
            else:
                excellent_words = all_words.filter(Q(quality__lte=100) & Q(quality__gt=50)).order_by('-count')[:5]
                good_words = all_words.filter(Q(quality__lte=50) & Q(quality__gt=20)).order_by('-count')[:10]
                bad_words = all_words.filter(Q(quality__lte=20) & Q(quality__gte=0)).order_by('-count')[:10]
                words = list(chain(excellent_words, good_words, bad_words))  # List of filtered words by quality and count
            test = Test()
            ...
            return render(request, 'contact.html', {'test_id': test.id, 'words': res_words})
    else:
...
def check_test(request, pk):
    if request.method == 'POST':  # If the form has been submitted...
        ...
        for word in right_words:
            all_words.filter(id=word.id).update(count=F('count') + 1)  # Update word's count
            user_answer = request.POST.get(word.foreign_word[0])
            right_answer = word.native_word[0]
            if user_answer == right_answer:
                temp_result += 1
                all_words.filter(id=word.id).update(quality=(F('quality') + 100) / 2)  # Increase word's quality
            else:
